...
import { Clock, Target, Briefcase, Dumbbell, Lightbulb, BookOpen, Brain, CheckCircle, X } from 'lucide-react';

const MissionCard = ({ mission, currentTheme, categories, onClick, isCompleted = false }) => {
  ...
  const getCategoryIcon = (categoryId) => {
    switch (categoryId) {
      ...
      case 'adaptabilite': return <Brain className="w-3 h-3" />;
  ...
  return (
    <div className="relative">
      ...
      {/* Croix stylisée intégrée pour les missions complétées */}
      {isCompleted && (
        <div 
          className="absolute -top-1 -right-1 w-6 h-6 rounded-full flex items-center justify-center shadow-lg"
          style={{ 
            backgroundColor: currentTheme.primaryColor,
            border: `2px solid ${currentTheme.backgroundColor}`,
            boxShadow: `0 0 10px ${currentTheme.primaryColor}60`
          }}
        >
          <CheckCircle className="w-4 h-4" style={{ color: currentTheme.backgroundColor }} />
        </div>
      )}
    </div>
  );
